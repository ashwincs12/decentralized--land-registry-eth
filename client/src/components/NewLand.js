...
export default function NewLand({state}) {
  ...
  const submitHandler = async (event) => {
    ...
    const tx = await contract.registerNewLand(
      surveyNo,district,taluk,village,blockNo,price,area);
    await tx.wait();
  };
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={4}>
              <TextField
                id="filled-basic"
                label="Area"
                helperText="Provide the land area (in cent)"
                variant="outlined"
